...
def replace_chunks(old_chunks, new_chunks, data, name):
    ...
    if len(old_chunks) != len(new_chunks):
        raise ValueError(f"{name}: replace_chunks requires old_chunks and new_chunks to have same length ({len(old_chunks)} != {len(new_chunks)})")

    # Build pattern: capture bytes between chunk boundaries for all but last chunk
    ...
    def replace_match(match):
        # Reconstruct replaced content: new_chunks[i] + the captured glue for i in range(...)
        replaced = b''.join([new_chunks[i] + match.group(i + 1) for i in range(len(new_chunks) - 1)])
        replaced += new_chunks[-1]
        try:
            preview = b"".join(old_chunks)[:16].hex().upper()
        except Exception:
            preview = "<hex preview failed>"
        print(f'{name} public key patched {preview}...')
        return replaced

    return re.sub(pattern, replace_match, data)

def replace_key(old, new, data, name=''):
    # Split into 4-byte chunks and replace
    if len(old) != len(new):
        raise ValueError(f"{name}: old and new keys must be same length ({len(old)} != {len(new)})")

    # First pass: 4-byte aligned chunks
    ...
    # Second pass: byte-by-byte permuted mapping using key_map
    key_map = [28,19,25,16,14,3,24,15,22,8,6,17,11,7,9,23,18,13,10,0,26,21,2,5,20,30,31,4,27,29,1,12]
    # Ensure indices in key_map are valid for current key length
    max_index = max(key_map)
    if max_index >= len(old):
        print(f"{name}: skipping byte-per-byte mapping because key length {len(old)} <= max key_map index {max_index}")
    else:
        try:
            old_chunks_b = [bytes([old[i]]) for i in key_map]
            new_chunks_b = [bytes([new[i]]) for i in key_map]
            data = replace_chunks(old_chunks_b, new_chunks_b, data, name)
        except IndexError:
            print(f"{name}: IndexError building byte-per-byte chunks — skipping this phase")

    # Architecture-specific arm64 reorder & code replacements
    arch = os.getenv('ARCH') or 'x86'
    arch = arch.replace('-', '')
    if arch == 'arm64':
        # repeat 4-byte-chunk replacement (safe because old/new lengths already checked)
        old_chunks = [old[i:i+4] for i in range(0, len(old), 4)]
        new_chunks = [new[i:i+4] for i in range(0, len(new), 4)]
        # reorder bytes specific to ARM64 (as in original script)
        try:
            ...
            # If old_chunks/new_chunks too short, skip this architecture-specific tweak
            print(f"{name}: arm64-specific byte reorder skipped due to insufficient key length")
    return data
...
def find_7zXZ_data(data: bytes):
    ...
    while b'\xFD7zXZ\x00\x00\x01' in _data:
        idx = _data.index(b'\xFD7zXZ\x00\x00\x01')
        offset1 = offset1 + idx + 8
        _data = _data[idx + 8:]
    offset1 -= 8
    offset2 = 0
    _data = data
    while b'\x00\x00\x00\x00\x01\x59\x5A' in _data:
        idx = _data.index(b'\x00\x00\x00\x00\x01\x59\x5A')
        offset2 = offset2 + idx + 7
        _data = _data[idx + 7:]
