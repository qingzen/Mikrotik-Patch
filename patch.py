...
def replace_chunks(old_chunks, new_chunks, data, name):
    # Safety checks
    if not old_chunks:
        print(f'[WARN] replace_chunks: no old_chunks provided for {name}, skipping.')
        return data
    if len(old_chunks) != len(new_chunks):
        raise ValueError(f'[ERR] replace_chunks: old/new chunk count mismatch for {name}: {len(old_chunks)} != {len(new_chunks)}')

    # Build regex pattern: escape each chunk and capture up to 6 bytes between them
    pattern_parts = []
    for chunk in old_chunks[:-1]:
        pattern_parts.append(re.escape(chunk) + b'(.{0,6})')
    ...
    def replace_match(match):
        groups = match.groups()
        expected_groups = len(new_chunks) - 1
        if len(groups) != expected_groups:
            # Unexpected — refuse to do a broken replacement
            print(f'[WARN] replace_match: unexpected capture group count for {name} (got {len(groups)}, expected {expected_groups}), skipping this match')
            return match.group(0)

        # Re-compose using new_chunks interleaved with the captured bytes
        replaced = b''
        for i in range(expected_groups):
            replaced += new_chunks[i] + groups[i]
...
def replace_key(old, new, data, name=''):
    if not old or not new:
        print(f'[WARN] replace_key: empty old/new key for {name}, skipping')
        return data

    # first pass: 4-byte chunk replacement
    ...
    try:
        ...
        print(f'[WARN] replace_key (first pass) for {name}: {e}')

    # second pass: byte-order mapped single-byte replacements using key_map
    key_map = [28,19,25,16,14,3,24,15,22,8,6,17,11,7,9,23,18,13,10,0,26,21,2,5,20,30,31,4,27,29,1,12,]
    max_index = max(key_map)
    if len(old) <= max_index or len(new) <= max_index:
        print(f'[WARN] replace_key: key length too short for mapped replacement for {name} (need index up to {max_index}) - skipping mapped step')
        return data
    ...
    try:
        ...
        print(f'[WARN] replace_key (mapped pass) for {name}: {e}')

    # arch-specific arm64 handling (same as original)
    arch = os.getenv('ARCH') or 'x86'
    arch = arch.replace('-', '')
    if arch == 'arm64':
        old_chunks = [old[i:i+4] for i in range(0, len(old), 4)]
        new_chunks = [new[i:i+4] for i in range(0, len(new), 4)]
        # assemble expected old_bytes order used on arm64
        try:
            ...
            print(f'[WARN] replace_key: arm64 chunk indices out of range for {name}, skipping arm64 reorder')
            return data
        ...
        if old_bytes in data:
            ...
            try:
                ...
                print(f'[WARN] replace_key (arm64 codes) for {name}: {e}')
    return data
